...
public class Outstanding_Fee_Report extends Login{
	
	@Test
	public void Outstanding_Fee_Reports() throws InterruptedException {
	  ...
	  Actions actions = new Actions(driver);
		  ...
		  WebElement Reports = 	driver.findElement(By.xpath("//a[@href='#/reports/reports-lists']"));
			...
			zz.executeScript("arguments[0].click()", Reports);
			   ...
			   Thread.sleep(6000);
				driver.findElement(By.xpath("(//div[@tabindex='-1'])[3]")).click();
						   Thread.sleep(2000);
    	...
    	for(WebElement ofccheckbox:ofccheckboxs2 ) {
    		if(ofccheckbox.getText().equalsIgnoreCase("F_Batch")) {
    			Thread.sleep(4000);
    			ofccheckbox.click();
    		}
    	}Thread.sleep(2000);
		driver.findElement(By.xpath("//span[text()=' Save ']")).click();
           ...
           driver.findElement(By.cssSelector("mat-select[formcontrolname='studentStatus']")).click();
		   Thread.sleep(3000);

			UM.selectOptionByText(driver,  "Active");
			   Thread.sleep(3000);

	            actions.sendKeys(Keys.TAB).perform();
			   Thread.sleep(3000);

				driver.findElement(By.xpath("//input[@formcontrolname='dateFrom']")).sendKeys(today.format(formatter));
				   Thread.sleep(3000);
